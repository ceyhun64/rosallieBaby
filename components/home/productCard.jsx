...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="flex-none p-2 w-full max-w-sm mx-auto">
      <Link
        href={`/all_products/${product.category}/${product.id}`}
        className="block"
      >
        <div
          className="bg-[#f9f8f4] border-[3px] border-[#e7e4db] rounded-xl overflow-hidden relative cursor-pointer"
        ...
        >
          ...
          <div
            className={`${
              isMobile ? "p-2" : "p-4"
            } text-center h-24 flex flex-col justify-between`}
          >
            <p
              className={`${
                isMobile ? "text-xs" : "text-sm md:text-base"
              } text-gray-700 mb-2 font-['Arial'] line-clamp-2`}
            >
              {product.description}
            </p>
